...
class PackCLRNetInputs(BaseTransform):
    ...
    def convert_targets(self, results):
        ...
        lanes = (
            np.ones(
                (self.max_lanes, 2 + 1 + 1 + 2 + self.n_offsets),
                dtype=np.float32,
            )
            * -1e5
        )
...
@TRANSFORMS.register_module()
class PackLaneATTInputs(BaseTransform):
    def __init__(
        self,
        # keys=None,
        meta_keys=None,
    ...
    def convert_targets(self, results):
        ...
        lanes = (
            np.ones(
                (self.max_lanes, 2 + 1 + 2 + self.n_offsets), dtype=np.float32
            )
            * -1e5
        )
        lanes[:, 0] = 1
        ...
        for lane_idx, lane in enumerate(old_lanes):
            ...
            lanes[lane_idx, 5 : 5 + len(all_xs)] = all_xs  # xs, absolute
